...
# Download
$(BUILD_DIR)/%/.download:
	echo "package.mk download target step1 mkdir $(@D)"
	mkdir -p $(@D)
	echo "package.mk download target step2 mkdir dldir: $(DL_DIR)"
	mkdir -p $(DL_DIR)
	echo "package.mk download target [] is a test in bash and dollarsign() is for var sub"
	echo "you should see files downloaded to dl subdir"
	[ -z "$($(PKG)_TARBALL_SRC)" -o -f $($(PKG)_DOWNLOAD_DST) ] || (cd $(DL_DIR) && curl --retry 5 -# -L -k -o $($(PKG)_TARBALL_DST) $($(PKG)_DOWNLOAD_URL))
	touch $@

# Untar and patch
$(BUILD_DIR)/%/.tar:
	echo "package.mk build and patch removing directory $(PKG_BUILD_DIR)/tar/"
	-rm -rf $(PKG_BUILD_DIR)/tar/
	echo "package.mk creating directory $(PKG_BUILD_DIR)/tar/$($(PKG)_NAME)-$(PKG_PKG_VERSION)$(BIGTOP_BUILD_STAMP)"
...
# Make source RPMs
$(BUILD_DIR)/%/.srpm:
	echo "package.mk building source rpms removing directory $(PKG_BUILD_DIR)/rpm"
	-rm -rf $(PKG_BUILD_DIR)/rpm/
	echo "package.mk building source rpms creating directory $(PKG_BIULD_DIR)/rpm"
	mkdir -p $(PKG_BUILD_DIR)/rpm/
	echo "package.mk build sourcerpm recursive copy dir $(BASE_DIR)/bigtop-packages/src/rpm/$($(PKG)_NAME)/ to $(PKG_BUILD_DIR)/rpm/"
	cp -r $(BASE_DIR)/bigtop-packages/src/rpm/$($(PKG)_NAME)/* $(PKG_BUILD_DIR)/rpm/
	echo "package.mk build sourcerpm making dir $(PKG_BUILD_DIR)/rpm/INSTALL,SOURCES,BUILD,SRPMS for RPMBULD command!!!!!!!"
	mkdir -p $(PKG_BUILD_DIR)/rpm/{INSTALL,SOURCES,BUILD,SRPMS}
	echo "package.mk build sourcerpm testing directory $($(PKG_TARBALL_SRC))"
	[ -z "$($(PKG)_TARBALL_SRC)" ] || cp $($(PKG)_DOWNLOAD_DST) $(PKG_BUILD_DIR)/rpm/SOURCES
	echo "package.mk testing -d file is directory "
	...
	echo "bigtop.mk build source rpm calling rpmbuild !!!!!!!!!!!!!!!"
	echo "bigtop.mk build source rpm rpmbuild parameter PKG_NAME_FOR_PKG:$(subst -,_,$($(PKG)_NAME))"
...
# Make binary RPMs
$(BUILD_DIR)/%/.rpm:
	echo "package.mk rpm build binaryrpms calling rpmbuild PKG_RELEASE_DIST:$(PKG)_RELEASE_DIST"
	echo "we need to look inside the spec file for the binary rpm build"
	$(PKG)_RELEASE_DIST=$(shell rpmbuild --eval '%{?dist}' 2>/dev/null); \
	...
	rpmbuild --define "_topdir $(PKG_BUILD_DIR)/rpm/" \
						...
						--rebuild $${SRCRPM}
	echo "package.mk build binary rpms recursive copy from $(PKG_BUILD_DIR)/rpm/RPMS/*/* to $($(PKG)_OUTPUT_DIR)"
